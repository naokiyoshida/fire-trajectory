...
- `npm run sim -- --check`: engine の既定シナリオ出力と、現行 Sheets
  ...
  - **前提**: パリティ相手は「現行 GAS で再生成済みのシミュレーション
    シート」であること。監査修正（実質割戻し・退職後社会保険料）以前の
    GAS で生成された**古いシートは仕様非忠実なので意図的に乖離する**
    （engine が正しい）。比較前に Sheets メニューの `setupSimulation()` を
    再実行してシートを現行仕様で作り直すこと。
