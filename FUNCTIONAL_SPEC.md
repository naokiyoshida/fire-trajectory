...
- **FIRE可能時期** = 予想資産(H) が必要ライン(I) を**以降ずっと上回り続ける
  最初の月**（初接触ではない。一時的に超えても後で割り込むなら不可）。
- **verdict（盤石/余裕/達成/未達）** は「fireDate に退職した場合の終了年齢
  資産（年金のみ・退職済み支出・一時金なし）」で判定し、fireDate と矛盾
  しない（fireDate があれば 未達 にはならない）。就労プランの終了資産は
  参考値として別途保持・表示（負＝早期退職が有利のサイン）。
...
- `app/sim/render-html.ts`: engine.ts のソースを **`typescript` の
  ...
  ため**手描き Canvas**（2本線＋Y/X 軸目盛りと値・0起点・マウスオーバー
  値表示・FIRE可能/枯渇の縦ラベル・DPI 対応・リサイズ追従）。スライダー
