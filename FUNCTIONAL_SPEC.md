...
| 共通 | NISA比率 | pct | 1.0（§4.1a。分配のうち NISA 非課税割合。実測=分配源は全額 NISA→ドラッグ0。設定行があれば優先） |
...
等価）。ただし**課税は非対称**：分配は受取時に国内 20.315% 課税、値上がり益は売却まで
非課税繰延。NISA（非課税口座）はこの国内課税が非課税。これを次式の**税ドラッグ**で表す。

- 税ドラッグ（年・名目）`d = 分配金利回り × (1 − NISA比率) × 0.20315`
  - `分配金利回り` は**総資産に対する**課税分配の割合（例: 本ポートフォリオで VIG・
    EDV・高配当ファンド計 ≒324千円/年 ÷ 総資産29.25M ≒ 1.1%）
  - `NISA比率` は**分配金のうち** NISA で非課税受取になる割合（資産割合ではない。
    分配はリスク資産に偏在し DC 年金は分配ゼロのため、分配額ベースで効かせる）
- 実効名目利回り `n' = 運用利回り_名目 − d` を §4.1 の `n` の代わりに用いる
- `分配金利回り = 0` または `NISA比率 = 1` で `d = 0` ＝従来の総リターン1本と一致（後方互換）

`n'` から導く `r_m` を青（期末資産）・橙（I列）・前進シミュすべてに一本で効かせるため、
線間の整合（§4.3）は保たれる。外国源泉（米株 ≒10%、NISA でも引かれる）は v1 では
未モデル化。実値（分配利回りレンジ・NISA比率）は MF 保有明細から設定する。
