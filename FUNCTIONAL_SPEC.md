...
- この自己整合化により engine はレガシー GAS シミュとは**設計上意図的に
  乖離**する。engine が唯一の正であり GAS シミュは撤去対象（§4.5）。
  `--check` はレガシー比較の**参考情報**であり撤去ゲートではない。
...
- **経緯**: かつて engine⇔現行 GAS シミュは月次 ¥0 で一致し、`--check` を
  「Sheets 撤去 OK」のゲートとしていた（commit b702208 で達成）。その後
  §4.3 の**自己整合化**（I列＝退職済み前提支出／sustained 判定／verdict
  整合）を採用し、engine はレガシー GAS シミュとは**設計上意図的に乖離**
  する状態へ移行した。
- したがって `npm run sim -- --check`（= `npm run sim:check`）は**撤去
  ゲートではなく参考情報**。レガシー GAS シミュとの差分を表示するが乖離
  しても exit 0（自己整合モデル移行による設計どおりの差。engine が正）。
  engine の正しさは `tests/sim/engine.test.ts`（fireNeedValue 一致・
  自己整合の各不変条件）が固定する＝唯一の正。
- Sheets シミュレーション/FIRE射程の **GAS 依存部は撤去対象**。撤去は
  別コミットで段階的に（取引履歴・資産推移の蓄積シートと、入力ストアで
  ある設定シートは残す）。FIRE射程レポートが シミュレーションシートを
  参照しているため、撤去時は「FIRE射程を engine 出力由来へ移植 or 同時
  撤去」を先に決める（§9.7 / レビュー lane5）。実施時期はユーザー判断。
