...
> **モデル**: FIRE 可能時期＝「その月に退職した場合、年金のみ・退職済み支出・一時金なしで前進シミュした終了年齢資産が `fireTargetRemain` 以上になる最初の月」。verdict は同一の前進シミュ（fireEndAssets）で判定するので fireDate と必ず整合（自己整合・§4.3）。就労プランの E列が FIRE後に I列を割り込んでも fireDate は揺らがない（旧「以降ずっと割り込まない」ルールは I列の意味と矛盾し撤廃）。「現行プラン（就労継続）の終了資産」も併記され、負なら早期退職が有利のサイン。
