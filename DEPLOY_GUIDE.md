...
**`npm run sim:check`**（= `npm run sim -- --check`）: engine の出力をレガシー「シミュレーション」シートと月次比較し差分を表示します。**かつては撤去ゲートでしたが、現在は撤去ゲートではなく参考情報**です。engine は §4.3 の自己整合化（I列＝退職済み前提支出／FIRE可能時期は sustained 判定／verdict 整合）により**レガシー GAS シミュとは設計上意図的に乖離**します（engine が正）。乖離があっても **exit 0**。engine の正しさは `npm test`（`tests/sim/engine.test.ts`）が固定します＝唯一の正。

> Sheets シミュレーション/FIRE射程の GAS 依存部は撤去対象です。撤去は別コミットで段階的に行い、取引履歴・資産推移・設定シートは残します。FIRE射程レポートが シミュレーションシートを参照しているため、撤去時は「FIRE射程を engine 出力由来へ移植」か「FIRE射程も同時撤去」を先に決めてください。実施時期はユーザー判断（パリティは役目を終えています）。
