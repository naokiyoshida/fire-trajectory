...
## 8.6 取引 ID を occurrence 付きハッシュへ移行（重複追記の解消・一度きり）

取引 ID の計算式から「大項目/中項目（カテゴリ）」を除外し、代わりに自然キー `日付・内容・金額・保有金融機関` 衝突時の**出現順 occurrence(0,1,2…)** を加える方式にしました。

- カテゴリ込みハッシュ（旧方式）はカテゴリを直すたびに同一取引が別 ID となり二重追記されていた → category を除外して解消。
- 一方でカテゴリ除外だけだと「同日・同額・同口座・同内容の別取引」（例: 同じ日に ¥10,000 の ATM 出金を2回）が同一 ID に衝突して2件目が取りこぼされる → **occurrence で別 ID に分離して両方保持**。
- occurrence は「スクレイプ時の MF 返却順」「移行時のシート追記順」のどちらでも決定的に同じ値になるため、二重追記も防げます。

**既存行の ID は旧方式のまま残る**ため、移行しないと次回 `npm run sync` で同じ取引が新 ID と一致せず再度追記されます。下記を **一度だけ** 実行してください。
...
「再計算で ID が変わる行: N」と、同一自然キーを持つグループ（occurrence で分離済み）の一覧が表示されます。

```powershell
# 2) 実行（取引履歴!A 列の ID を新方式で振り直す）
npm run remap-ids
```

- ID 列のみを書き換えます。日付・金額などの取引データには触れません。
- 同一自然キーの取引は `ℹ` で行番号・occurrence・取得日時・category が一覧表示されますが、occurrence で別 ID に分離済みなので**両方そのまま保持**されます。**手作業の削除や手動補正は不要**です。
- 以前の版で「衝突＝重複なので片方削除」と案内していましたが、occurrence 方式により**実取引の取りこぼしも二重追記も起きない**ため、その手順は不要になりました。
- **残余エッジ**: Money Forward が同一自然キー取引の相対並び順を run 間で入れ替えた場合のみ、当該取引が一度だけ再追記され得ます（稀。発生しても `取引履歴` 上で目視確認でき、無言の取りこぼしより安全側）。
