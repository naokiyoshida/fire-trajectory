...
> **前提（重要）**: パリティ相手は「**現行 GAS で再生成済み**のシミュレーションシート」であること。監査修正（年金/一時金の実質割戻し・退職後社会保険料の計上）**以前**の GAS が生成した古いシートは仕様非忠実で、engine とは**意図的に乖離**します（この場合 engine が正）。`--check` を撤去ゲートとして信頼する前に、Sheets メニューから `setupSimulation()` を実行し、現行仕様でシートを作り直してから比較してください。再生成前の `exit 6` は engine のバグを意味しません。
