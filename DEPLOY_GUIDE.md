...
その後、Sheets を開いてメニューバーの **Fire Trajectory → 設定シートの再構築** を実行（GAS が入力ストアの `設定` を作成）。続けて **Fire Trajectory → レポートの再構築** を実行（`月次収支`、`カテゴリ別支出`、`純資産推移`、`資産配分` を作成）。FIRE 月次予測は Sheets ではなく `npm run sim` が生成する `dist/fire.html` で確認します（§9.7）。
...
6. メニュー **Fire Trajectory → 設定シートの再構築** を実行 → `設定` 作成
7. メニュー **Fire Trajectory → レポートの再構築** を実行 → `月次収支` / `カテゴリ別支出` / `純資産推移` / `資産配分` 作成
8. 手順2で作ったダミーシートを削除

これで全 8 シートが日本語名で揃う（取引履歴 / 資産推移 / 手動入力資産 / 設定 / 月次収支 / カテゴリ別支出 / 純資産推移 / 資産配分）。FIRE 月次予測は `dist/fire.html`（§9.7）。タスクスケジューラの設定変更は不要（PowerShell スクリプトはシート名に依存しない）。
...
**`npm run sim`**（読み取り専用）: 設定シートを読み、`dist/fire.html` を生成します。ブラウザで `file://…/dist/fire.html` を開くとスライダーで設定を動かしながら「期末資産 vs FIRE必要資産」グラフ（Y は 0起点・期末資産に合わせてスケールし可視域 [0, top] の外＝序盤の巨大な FIRE必要資産も枯渇後の負の期末資産も描かず途切れさせる〔偽の水平線を出さない・枯渇は赤い縦ラベルで明示〕、X は描画幅に応じ数年おきに間引き、マウスオーバーで全系列の生値、FIRE可能/枯渇ラベル）と FIRE 可能時期がリアルタイム更新されます（完全ローカル・外部送信なし）。`npm run sync` 末尾でも best-effort で自動再生成（失敗しても sync は成功扱い）。`dist/` は gitignore。

> **モデル**: FIRE 可能時期＝「その月に退職した場合、年金のみ・退職済み支出・一時金なしで前進シミュした終了年齢資産が `fireTargetRemain` 以上になる最初の月」。verdict は同一の前進シミュ（fireEndAssets）で判定するので fireDate と必ず整合（自己整合・§4.3）。就労プランの E列が FIRE後に I列を割り込んでも fireDate は揺らがない（旧「以降ずっと割り込まない」ルールは I列の意味と矛盾し撤廃）。インフレ前提は青（期末資産）と橙（I列）で統一＝支出は実質固定（÷deflator しない・生活費は購買力一定）、年金のみ名目固定。両線が同一の漸化式になり傾き挙動が整合（旧 I列だけ支出を÷deflator する非対称＝高インフレで橙が逆傾き等の不整合は撤去）。「現行プラン（就労継続）の終了資産」も併記され、負なら早期退職が有利のサイン。

> **計算の正は engine.ts のみ**。レガシー GAS シミュ／FIRE射程レポート／パリティ比較（`sim:check`）は撤去済み（§4.5）。engine の正しさは `npm test`（`tests/sim/engine.test.ts`・`tests/gas/contract.test.ts`）が固定します。自己診断は `dist/fire.html` 下部のデバッグ表（実質係数・年金実質列を含む）を貼り付ければ実質計算系の異常も特定できます。
