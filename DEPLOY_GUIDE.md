...
出力で「再計算で ID が変わる行: N」と、再計算後に同一 ID が重複する行数（過去に二重追記された実重複の件数）を確認します。
...
- `⚠ 再計算後に同一 ID が重複する行が N 行` と出た場合、それは過去にカテゴリ編集で二重追記された**実際の重複行**です。自動削除はしません。`取引履歴` シートで日付・内容・金額・保有金融機関が同一の行を目視で1行に整理してください（残すのは1行で可）。重複を消さなくても次回以降の二重追記は止まりますが、過去の集計値が膨らんだままになります。
- 整理後にメニュー **Fire Trajectory → レポートの再構築** を実行すると集計が正しい値で再計算されます。
